...
public class ByteArrayTokenizer extends AbstractTokenizer<byte[]> {
	/*
	...
	public void setSource(byte[] source, byte[] delimiters, DelimiterType type,
	...
	public IToken<byte[]> nextToken() throws IllegalStateException {
		...
		if (moreTokens) {
			...
			if (moreTokens)
				token = new ByteArrayToken(source, tsIndex, (teIndex - tsIndex));
		}
